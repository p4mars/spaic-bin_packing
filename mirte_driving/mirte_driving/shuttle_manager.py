...
class ShuttleManager(Node):
    def __init__(self):
        ...
        # Return-to-start: record the (random) drop pose once SLAM/TF is ready,
        # then drive back to it after the last leg.  return_home toggles it.
        self._return_home = bool(self.declare_parameter('return_home', True).value)
        self._home_pose = None        # (x, y, yaw) where the robot was dropped
        self._returning_home = False
        self._home_marker_pub = self.create_publisher(Marker, '/home_marker', 10)
        self._home_pose_pub = self.create_publisher(PoseStamped, '/zone_home', 10)
    ...
    def _home_marker(self):
        """Yellow sphere at the recorded drop pose, for RViz (lifetime 0 = stays
        on screen forever, so you can see the robot return to it)."""
        m = Marker()
        ...
        m.action = Marker.ADD
        m.pose.position.x = float(self._home_pose[0])
        m.pose.position.y = float(self._home_pose[1])
    ...
    def _home_pose_stamped(self):
        """The recorded drop pose as a PoseStamped on /zone_home, so RViz shows a
        home arrow exactly like /zone_a_pose and /zone_b_pose."""
    ...
    def _tick(self):
        ...
        # Keep the home marker visible in RViz once the drop pose is known.
        if self._home_pose is not None:
            self._home_marker_pub.publish(self._home_marker())
            self._home_pose_pub.publish(self._home_pose_stamped())

        if self._state == 'WAIT_SLAM':
            if self._robot_xy() is not None:
                self.get_logger().info('SLAM/TF ready — searching for zone markers.')
                if self._home_pose is None:                # record the drop pose once
                    self._home_pose = self._robot_pose()
                    self.get_logger().info(
                        f'Home recorded at ({self._home_pose[0]:.2f}, '
                        f'{self._home_pose[1]:.2f}).')
        ...
        if self._state == 'SHUTTLE':
            ...
            if self._leg >= len(self._legs):
                if self._return_home and self._home_pose is not None:
                    self.get_logger().info('Shuttle complete — returning home.')
        ...
        if self._state == 'RETURN_HOME':
            # Drive back to the recorded drop pose, then finish.  Reuses the same
            # Nav2 goal machinery as every leg; _goal_done ends the mission on
            # success.
            if self._navigating:
                if (now - self._goal_sent_ns) / 1e9 > self._goal_timeout:
                    self.get_logger().warn('Home goal timeout — retrying.')
                    self._cancel()
                    self._navigating = False
                return
            hx, hy, hyaw = self._home_pose
            self.get_logger().info(f'Returning home → ({hx:.2f}, {hy:.2f}).')
            self._send_goal(hx, hy, hyaw)
            return
    ...
    def _goal_done(self, fut, seq):
        ...
        if self._returning_home:
            # The home goal must NOT fall through to the leg logic below (it would
            # index self._legs[self._leg] out of range — the legs are done).
            if status == GoalStatus.STATUS_SUCCEEDED:
                self.get_logger().info('✓ Home reached — mission complete.')
                self._returning_home = False
                self._state = 'DONE'
            else:
                self.get_logger().warn('Home goal failed — retrying.')  # RETURN_HOME re-sends
            return
