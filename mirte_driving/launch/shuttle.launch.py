...
═══════════════════════════════════════════════════════════════════════════════
STARTUP SEQUENCE  (EVENT-DRIVEN — NO start timers.  Process-start order is the
launch's job; runtime readiness is shuttle_manager's job, via its WAIT_SLAM state.)
═══════════════════════════════════════════════════════════════════════════════
  AT LAUNCH (all together, none needs a delay):
    scan_filter, zone_detector, sim statics, DDS profile, slam_toolbox, tf_gate,
    and shuttle_manager.
      • slam_toolbox just waits for /scan_filtered to arrive.
      • shuttle_manager waits in WAIT_SLAM (map→base_link) and calls
        wait_for_server before sending any Nav2 goal — so starting it early is
        safe; it acts only once SLAM, then Nav2, are live.
  GATED ON A CONDITION (not a timer):
    Nav2 container — tf_gate exits the instant map→odom is live, and
    RegisterEventHandler(OnProcessExit) fires Nav2 right then.  Composed into one
    container so lifecycle activation is intra-process and can't time out (this is
    what removed the need for the old fixed 35 s delay).
...
def generate_launch_description():
    ...
    # ── Nav2, defined as a variable so an event handler can launch it the INSTANT
    #    map→odom is live (see the event handlers at the end), instead of a fixed
    #    timer.  COMPOSED: all five servers run in ONE container sharing a single
    #    DDS participant.  On this robot MIRTE_FASTDDS=false, so ~24 separate
    #    participants flooded multicast discovery and the lifecycle manager's calls
    #    timed out, aborting bringup.  In one container the lifecycle calls are
    #    intra-process (no DDS discovery) so they can't time out, and Nav2 is 1
    #    participant instead of 5.
    ...
    # The mission brain — launched just after Nav2 (event handler at the end).
    shuttle_manager_node = Node(
        package='mirte_driving', executable='shuttle_manager.py',
    ...
    # One-shot gate: exits when map→odom is live → the event handler releases Nav2.
    tf_gate = Node(package='mirte_driving', executable='tf_gate.py',
                   name='tf_gate', output='screen', parameters=[sim])
    ...
    return LaunchDescription(args + [
        ...
        # SYNC node, like the proven minimal_slam stack: processes every scan in
        # order (the async node DROPS scans under load and smeared the map).  No
        # start timer — it simply waits for /scan_filtered to arrive.
        Node(package='slam_toolbox', executable='sync_slam_toolbox_node',
             name='slam_toolbox', output='screen', parameters=[slam_params, sim]),
        # Polls for map→odom from the start and exits the instant it's live,
        # which (via the event handler below) releases Nav2.
        tf_gate,

        # Nav2 launches the INSTANT map→odom is live: tf_gate (started with SLAM
        # above) exits when the transform appears, and this fires nav2_container on
        # that exit — replacing the old fixed 35 s timer.  nav2_container is defined
        # as a variable above (see that comment for why Nav2 is composed).
        RegisterEventHandler(OnProcessExit(target_action=tf_gate,
                                           on_exit=[nav2_container])),

        # shuttle_manager: NO start timer.  It IS the state machine — it waits in
        # WAIT_SLAM for map→base_link and calls wait_for_server before any goal, so
        # it can start now and simply not act until SLAM (then Nav2) are live.  By
        # the time it leaves SEARCH and sends a goal, Nav2 (released by tf_gate) is
        # up; a goal that races activation is handled by wait_for_server + the
        # _goal_accepted retry.
