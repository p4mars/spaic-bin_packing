# README_NAV — Navigation & the A↔B Shuttle (`mirte_driving`)

Navigation half of `mirte_driving`. This document explains **how the robot
...
| `exploration_nav2_params.yaml` | config (Nav2 tuning) | **Project config**, modelled on the reference's `minimal_nav2_params.yaml` | The reference shipped only a minimal Nav2 config; this is the project's tuned, mission-specific version |
| `nav2_minimal_tree.xml` | config (behaviour tree) | **Reference config** (`mirte_navigation`), adapted | The reference shipped this tree; adapted here (e.g. no costmap clearing) |
...
Project-authored, modelled on the reference's `minimal_nav2_params.yaml` and tuned
for this robot.
...
ros2 launch mirte_driving detector.launch.py   # same network + ROS_DOMAIN_ID
```
